...
Paper Scraping And Mining Engine helps extract papers From ArXiv in LaTeX format and then parses the structure, mines the ontology, and extracts the tables. This processed information is stored in the Data Layer. 
...
A data processing pipeline using Metaflow\footnote{https://metaflow.org/} is employed to filter the subset of CS papers which are stored in the \nameref{sci-genie-core:data-layer:ss-index}. The pipeline further  correlated the citations of papers which are present on ArXiv and stores them in the \nameref{sci-genie-core:data-layer:ss-join-index}. 
% The Semani .
...
% The API Layer provides to help label the intent of various tables. 
% To create machine learning models which are able to detect tables describing comparsions,
% For the task of filtering tables which are describing comparisons;
